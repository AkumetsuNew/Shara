...
bool BaseCheck(int base, std::string numAll)
{
    ...
    {
        std::cout << "Основание должно быть больше нуля и меньше 36." << std::endl;
        return false;
    }
    return true;
}
...
bool IsValidNumeral(int baseOld, std::string numOld, std::string numAll)
{
    ...
    for (auto i = 0; i < numOld.length(); i++)
    {
        ...
        if (!isValidNumeral)
        {
            std::cout << "Введена цифра, не соответствующая основанию." << std::endl;
            return false;
        }
    }
    return true;
...
void Prog4_9()
{
    ...
    std::cout << "Введите число в заданном основании:" << std::endl;
    std::cin >> numOld;

    std::cout << "Введите основание новой системы счисления:" << std::endl;
    std::cin >> baseNew;
    ...
    std::cout << "\nЧисло " << numOld << "(" << baseOld << ") в основании "
        << baseNew << ":\n" << numNew << std::endl;
}